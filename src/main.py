...
print(f"Environment AI_ENGINE_PORT: {os.getenv('AI_ENGINE_PORT', '8000')}", file=sys.stderr)
...
async def lifespan(app: FastAPI):
    ...
    print("🚀 AI Engine lifespan startup", file=sys.stderr)
    ...
    try:
        ...
        print("✅ AI Engine components initialized", file=sys.stderr)
...
async def test_endpoint():
    ...
    return {
        ...
        "timestamp": datetime.now().isoformat(),
        "host": "0.0.0.0",
        "port": os.getenv("AI_ENGINE_PORT", "8000")
    }
...
if __name__ == "__main__":
    host = "0.0.0.0"  # Belangrijk: luister op alle interfaces, niet localhost
    port = int(os.getenv("AI_ENGINE_PORT", "8000"))
    
    print(f"🌐 Starting server on {host}:{port}", file=sys.stderr)
    print(f"📁 Working directory: {os.getcwd()}", file=sys.stderr)
    print(f"🐍 Python executable: {sys.executable}", file=sys.stderr)
    print(f"📦 Installed packages: {[p for p in sys.modules.keys() if not p.startswith('_')][:20]}...", file=sys.stderr)
